...
export default async function PinPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!pin) notFound();
  // If this is a catalog product (has SKU), send user to the product page.
  ...
  return (
    <div className="max-w-3xl mx-auto px-2 pt-3">
      <div className="relative overflow-hidden rounded-pin bg-[#f3f3f3]">
        {pin.mediaType === "video" ? (
        ...
        <Link
          href="/feed"
          aria-label="Назад"
          className="absolute top-3 left-3 z-10 w-12 h-12 grid place-items-center rounded-2xl bg-white/95 backdrop-blur shadow-sm hover:bg-white"
        >
